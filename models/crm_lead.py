...
class SalesTeam(models.Model):
    ...
    def get_tiles_data(self, time_interval):
        ...
        invoices = self.env['account.move'].search([('create_date', '>=',
                                                     start_date),
                                                    ('create_date', '<=',
                                                     end_date),
                                                    ('user_id', '=',
                                                     self.env.user.id)])
        revenue = sum(invoices.filtered(lambda r: r.currency_id ==
                                                  company_id.currency_id).
                      mapped('amount_total'))

        lost_leads = my_leads.search([('probability', '=', 0), ('active', '=',
                                                                False),])
        ...
        print('leads_by_month_data', leads_by_month_data)
